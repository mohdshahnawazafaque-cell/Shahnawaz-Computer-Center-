    ...
    >
      ...
      <div className="bg-gradient-to-r from-[#0B2545] via-[#133A6B] to-[#0B2545] text-white rounded-2xl p-5 sm:p-7 shadow-lg border border-[#1d4677] relative overflow-hidden mb-6">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="bg-white dark:bg-slate-800/10 backdrop-blur-sm border border-white/15 p-3 sm:p-4 rounded-xl flex-shrink-0 flex flex-col items-center sm:items-start text-center sm:text-left gap-2">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 sm:p-5 border border-slate-200 dark:border-slate-700 shadow-sm mb-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
          <div className="relative w-full sm:flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-2.5 bg-slate-50 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-xl text-xs sm:text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-red-600 focus:bg-white dark:bg-slate-800 transition-all font-medium"
            />
            {searchQuery && (
              <button
                type="button"
                onClick={() => setSearchQuery('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 dark:text-slate-300 rounded-full hover:bg-slate-200"
              >
          ...
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800/50 p-1 rounded-xl w-full sm:w-auto overflow-x-auto">
            <button
              type="button"
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1.5 cursor-pointer flex-shrink-0 border ${
                    isSelected
                      ...
                      : 'bg-slate-50 dark:bg-slate-700 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:bg-slate-800/50 hover:border-slate-300 dark:border-slate-600'
                  }`}
                >
                  <span>{cat}</span>
                  <span
                    className={`text-[10px] px-1.5 py-0.2 rounded-full font-black ${
                      isSelected ? 'bg-white dark:bg-slate-800/20 text-white' : 'bg-slate-200 text-slate-600 dark:text-slate-300'
                    }`}
                  >
                    {count}
                  </span>
      ...
      {filteredItems.length === 0 ? (
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-10 text-center border border-slate-200 dark:border-slate-700 shadow-sm max-w-md mx-auto my-6 space-y-3">
          <HelpCircle className="w-12 h-12 text-slate-400 mx-auto" />
          <h3 className="text-base font-bold text-slate-900 dark:text-white">No Schemes or Services Found</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {filteredItems.map((item) => (
            <div
              ...
              className="bg-white dark:bg-slate-800 rounded-2xl border-2 border-slate-200 dark:border-slate-700 hover:border-blue-600 hover:shadow-md transition-all p-4 sm:p-5 flex flex-col justify-between group"
            >
              <div>
                {/* Card Badges */}
                ...
                {/* Scheme Title */}
                <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white group-hover:text-blue-900 leading-snug font-sans">
                  {item.name}
                </h3>
