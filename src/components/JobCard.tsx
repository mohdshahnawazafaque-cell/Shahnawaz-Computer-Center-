...
export const JobCard: React.FC<JobCardProps> = ({ post, onClick, layout = 'grid' }) => {
  ...
  if (layout === 'sarkari_list') {
    ...
    return (
      <div
        ...
        onClick={onClick}
        className="group px-2.5 py-1.5 hover:bg-red-50/60 border-b border-slate-200 dark:border-slate-700/80 transition-colors cursor-pointer flex items-start justify-between gap-1.5 select-none"
      >
        <div className="flex items-start gap-1.5 min-w-0 flex-1">
          <span className="text-slate-800 dark:text-slate-100 font-black text-sm leading-tight select-none shrink-0">•</span>
  ...
  if (layout === 'compact') {
    return (
      <div
        ...
        onClick={onClick}
        className="p-3 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 hover:border-blue-400 rounded-xl transition-all cursor-pointer group flex items-center justify-between gap-3 shadow-xs"
      >
        <div className="flex items-start gap-2.5 min-w-0 flex-1">
          <div className="mt-0.5 p-1.5 bg-slate-100 dark:bg-slate-800/50 rounded-lg group-hover:bg-blue-100 transition-colors flex-shrink-0">
            {getPostIcon()}
          </div>
          <div className="min-w-0 flex-1">
            ...
            <h4 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white group-hover:text-blue-900 line-clamp-1">
              {post.title}
            </h4>
          </div>
        </div>
  ...
  return (
    <article
      ...
      onClick={onClick}
      className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-blue-400 hover:shadow-md transition-all duration-200 flex flex-col justify-between p-4 cursor-pointer group relative overflow-hidden"
    >
      {/* Top badges bar */}
      <div>
        <div className="flex flex-wrap items-center justify-between gap-1.5 mb-2">
          ...
          {post.state && (
            <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-0.5 bg-slate-50 dark:bg-slate-700 px-1.5 py-0.5 rounded border border-slate-150">
        ...
        {/* Title */}
        <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white group-hover:text-[#0B2545] leading-snug tracking-tight line-clamp-2 mb-2 font-sans">
          {post.title}
        </h3>

        {/* Metadata summary */}
        <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-300 mb-3 bg-slate-50 dark:bg-slate-700/80 p-2.5 rounded-lg border border-slate-150">
          {post.department && (
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] pt-1 border-t border-slate-200 dark:border-slate-700/60">
            {post.totalVacancy ? (
              ...
              <span className="text-slate-500 dark:text-slate-400">Regular Update</span>
            )}
