...
export const AIChatWidget: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-4 left-4 z-50 flex flex-col items-start print:hidden">
      ...
      <div
        className={`absolute bottom-16 left-0 mb-4 w-[90vw] max-w-[360px] sm:w-[380px] bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden transition-all duration-300 origin-bottom-left ${
      ...
      >
        ...
        <div className="bg-gradient-to-r from-blue-700 to-indigo-700 p-4 text-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-white dark:bg-slate-800/20 flex items-center justify-center backdrop-blur-sm shadow-inner">
          ...
          <button onClick={() => setIsOpen(false)} className="p-1.5 hover:bg-white dark:bg-slate-800/20 rounded-full transition-colors text-blue-100 hover:text-white">
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Chat Area */}
        <div className="flex-1 overflow-y-auto p-4 bg-slate-50 dark:bg-slate-700 flex flex-col gap-4">
          {messages.map((msg) => (
                ...
                <div 
                  className={`px-3.5 py-2.5 rounded-2xl shadow-sm text-sm ${
                    msg.role === 'user' 
                      ...
                      : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-tl-sm whitespace-pre-wrap'
                  }`}
                >
                  {msg.content}
                </div>
          ...
          {isLoading && (
            <div className="flex justify-start max-w-full">
              <div className="flex gap-2 max-w-[85%] flex-row">
                ...
                <div className="px-4 py-3 rounded-2xl shadow-sm text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-tl-sm flex items-center gap-1.5">
        ...
        <div className="p-3 bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700 shrink-0">
          <form onSubmit={handleSubmit} className="flex items-center gap-2 relative">
            <input
              ...
              className="flex-1 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-full pl-4 pr-11 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all text-slate-700 dark:text-slate-200 placeholder:text-slate-400"
              disabled={isLoading}
            />
            <button
              type="submit"
