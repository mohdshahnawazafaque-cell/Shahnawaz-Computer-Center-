...
export const Breadcrumbs: React.FC<BreadcrumbsProps> = ({ items, onNavigate }) => {
  return (
    <nav id="breadcrumb-navigation" aria-label="Breadcrumb" className="my-3 px-4 max-w-7xl mx-auto">
      <ol className="flex items-center flex-wrap gap-1 text-xs text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 p-2.5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-xs">
        ...
        {items.map((item, index) => {
          ...
          return (
            <li key={index} className="flex items-center gap-1 min-w-0">
              ...
              {isLast || !item.path ? (
                <span className="font-bold text-slate-800 dark:text-slate-100 truncate max-w-[200px] sm:max-w-md" aria-current="page">
                  {item.label}
                </span>
              ) : (
                <button
